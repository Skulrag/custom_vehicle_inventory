...
function VehicleMaxSpeed(vehicle, weight, maxweight)
    ...
    if percent > 80 then
        SetEntityMaxSpeed(vehicle, GetVehicleModelMaxSpeed(hashk) / 1.4)
    elseif percent > 50 then
        SetEntityMaxSpeed(vehicle, GetVehicleModelMaxSpeed(hashk) / 1.2)
    else
        SetEntityMaxSpeed(vehicle, GetVehicleModelMaxSpeed(hashk))
    end
end
]]
...
function openmenuvehicle()
    ...
    local typeVeh = GetVehicleClass(vehicle)
    ...
    for i = 1, #vehiclePlate do
        if (vehiclePlate[i].plate .. ' ') == globalplate then
            if vehiclePlate[i].job == 1 then
                Config.VehicleLimit[typeVeh] = 30000000
            else
                Config.VehicleLimit[typeVeh] = Config.VehicleLimitSave[typeVeh]
            end
            break
        end
    end
    if globalplate ~= nil or globalplate ~= "" or globalplate ~= " " then
        ESX.TriggerServerCallback('esx_truck:checkvehicle', function(valid)
            if (not valid) then
                ...
                if vehicle > 0 and closecar ~= nil and GetPedInVehicleSeat(closecar, -1) ~= GetPlayerPed(-1) then
                    lastVehicle = vehicle
                    ...
                    if ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'inventory') then
                        ...
                        if locked == 1 or class == 15 or class == 16 or class == 14 or class == 11 then
                            ...
                            if globalplate ~= nil or globalplate ~= "" or globalplate ~= " " then
                                ...
                                TriggerServerEvent("esx_truck_inventory:getInventory", GetVehicleNumberPlateText(vehicle))
                            end
                        else
                                        ...
                                        function(data4, menu4)
                                            ...
                                            if quantity > 0 and quantity <= tonumber(data3.current.count) and vehFront > 0 then
                                                ...
                                                if not max then
                                                    ...
                                                    TriggerServerEvent('esx_truck_inventory:addInventoryItem', GetVehicleClass(closecar), GetDisplayNameFromVehicleModel(GetEntityModel(closecar)), GetVehicleNumberPlateText(vehFront), data3.current.value, quantity, data3.current.name, data3.current.type)
