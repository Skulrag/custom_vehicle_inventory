            ...
            function(result)
                if result ~= nil and #result > 0 then
                    for _, v in pairs(result) do
                        ...
                        if v.job == 'transistep' then
                            table.insert(vehicules, { plate = vehicle.plate, job = 1 })
                        else
                            table.insert(vehicules, { plate = vehicle.plate, job = 0 })
                        end
                    end
                end
...
AddEventHandler('esx_truck_inventory:addInventoryItem', function(type, model, plate, item, qtty, name, itemType)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)

    if plate ~= nil then
        MySQL.Async.execute('INSERT INTO truck_inventory (item,count,plate,name,itemt) VALUES (@item,@qty,@plate,@name,@itemt) ON DUPLICATE KEY UPDATE count=count+ @qty',
                {
                    ['@plate'] = plate,
                    ['@qty'] = qtty,
                    ['@item'] = item,
